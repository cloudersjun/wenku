...
from scrapy_wk.handle_input import HandleInput
from scrapy_wk.handle_output import HandleOutput
from scrapy_wk.handler_parse import HandleParse
import scrapy_wk.spiders
import scrapy_wk.settings
from scrapy_wk.middlewares import downloadwebkit
...
class DmozSpider(Spider):
    ...
    def start_requests(self):
        ...
        for input_item in input_array:
            request = scrapy.Request(url=input_item, callback=self.parse, dont_filter=True)
            request.meta["item_info"] = input_item
            yield request

    def parse(self, response):
        parse = HandleParse(response)
        global ret
        ret += "\n#########################\n"
        ret += parse.parse()
        logging.info(json.dumps(ret, ensure_ascii=False, encoding="gb2312"))
    ...
    def close(self, reason):
        ...
        file_name = now.replace(" ", "_") + ".txt"
        logging.info(u"解析结果:" + json.dumps(ret, ensure_ascii=False, encoding="gb2312"))
        logging.info(u"最终写入文件：" + file_name)
        handle_output = HandleOutput(file_path, file_name, file_header, ret, input_array)
        handle_output.write()
